...
import com.coinbase.exchange.api.orderbook.OrderBook;
...
public class WebsocketFeed {
	...
	/**
	 * Callback hook for OrderBookMessage Events. This method will be invoked when a
			...
			SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
				...
				public Void doInBackground() {
					...
					if (type.equals("heartbeat")) {
						...
						// received orders are not necessarily live orders - so I'm ignoring these msgs
						// as they're
						// subject to change.
						...
						if (!message.getReason().equals("filled")) {
							OrderBookMessage doneOrder = getObject(json,
									new TypeReference<OrderDoneOrderBookMessage>() {
									});
							orderBook.updateOrderBook(doneOrder);
						}
					} else if (type.equals("match")) {
						log.info("Order matched: " + json);
						OrderBookMessage matchedOrder = getObject(json,
								new TypeReference<OrderMatchOrderBookMessage>() {
								});
						...
						// orderBook.updateOrderBookWithChange(getObject(json, new
						// TypeReference<OrderChangeOrderBookMessage>(){}));
					} else {
						// Not sure this is required unless I'm attempting to place orders
						// ERROR
						log.error("Error {}", json);
						// orderBook.orderBookError(getObject(json, new
						// TypeReference<ErrorOrderBookMessage>(){}));
					}
					return null;
				}
